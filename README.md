...
一个安静型大模型情报监控器：监控主流大模型厂商的新模型发布信息，并通过企业微信推送一条聚合后的高质量消息。默认策略是少打扰：同一新模型发布事件只推送一次主消息，后续只有超高影响力人物明确点名该模型时才补充推送。
...
- 厂商官方公告、官方文档或模型目录出现新模型发布信号
- 或多个可信来源交叉确认
- 事件等待 `EVENT_SETTLE_MINUTES` 后统一聚合推送

补充推送触发条件：

- 只在超高影响力人物明确点名某个模型时补充推送，例如 Elon Musk、Jensen Huang、Sam Altman、Dario Amodei、Demis Hassabis 等
- 不因为普通社区讨论、GitHub 搜索量、Hacker News 热度或媒体转述补充推送

默认不会因为普通媒体转述、社区零散讨论、重复榜单搬运而补推。系统会在主推送里尽量一次性包含评分和具体发布内容，后续只在超高影响力人物明确点名时再打扰你。
...
没有配置 `LLM_API_KEY` 时，系统仍会推送基于规则的摘要和评分；配置 LLM 后，摘要质量会更好，但系统会要求模型严格基于已采集来源生成，不写普通社区讨论、GitHub 搜索量、Hacker News 热度或未经确认的信息。
